...
def test_list_releases(db: None, client: Client, user: User) -> None:
    response = client.get(
        "/api/v1/releases/", headers={"authorization": "Key s3cretk3y"}
    )
...
def test_details_view(
    ...
    response = client.get(release_uri, headers={"authorization": "Key s3cretk3y"})
