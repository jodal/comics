...
class Crawler(CrawlerBase):
    ...
    def crawl(self, pub_date):
        date_to_index_page = self.parse_page('http://www.digitalpimponline.com/strips.php?title=movie')

        # Go through all IDs in the document, checking to see if the date is the date we want in the option drop-down
	...
	# (cheap conversion to a set to eliminate the duplicate IDs from different parts of the HTML to save time...)
	    ...
	    for the_date in possible_date_and_title:
                ...
                # penalty of ~500+ regex comparisons
                    if the_date == pub_date.strftime( r"%B %d, %Y" ).replace( " 0", " ", 1 ):
                        the_id = possible_id
                        break
